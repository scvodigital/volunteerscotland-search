...
export class ComponentsInitialiser {
  ...
  initialise() {
    ...
    $('[data-slider]').each((i, o) => {
        ...
        var valueHolder = $(o).attr('data-slider-target');
        slider.listen('MDCSlider:change', () => {
            var $ourvalholder = $(valueHolder);
            $ourvalholder.attr("value", slider.value);
        });
    ...
    $('[data-collapse-target]').off('click').on('click', (evt) => {
       ...
       */
      $('[data-checkbox-collapse-target]').off('click').on('click', (evt) => {
