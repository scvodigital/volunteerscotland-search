...
export class Auth {
  ...
  async changePassword(password, passwordConfirm, oldPassword) {
    try {
      if (password !== passwordConfirm) {
        throw new Error('The two passwords entered do not match');
      }
      ...
      if (oldPassword) {
        try {
          const authCred = firebase.auth.EmailAuthProvider.credential(currentUser.email, oldPassword);
          const userCred = await currentUser.reauthenticateWithCredential(authCred);
        } catch(err) {
          throw new Error('Failed to validate old password');
        }
      }
  ...
  async changeEmail(email, emailConfirm, password) {
    try {
      ...
      if (password) {
        try {
          const authCred = firebase.auth.EmailAuthProvider.credential(currentUser.email, password);
          const userCred = await currentUser.reauthenticateWithCredential(authCred);
        } catch(err) {
